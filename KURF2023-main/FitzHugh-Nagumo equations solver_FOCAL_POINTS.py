# -*- coding: utf-8 -*-
"""
Created on Sat May 27 12:11:25 2023

@author: olegadmin
"""

# FitzHugh-Nagumo (FN) equations solver for Focal points
# Libraries:
import numpy as np
import pylab as plt
...
All_Pwaves_Lii = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_Liii = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_WCT = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_V1 = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_V2 = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_V3 = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_V4 = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_V5 = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_V6 = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
All_Pwaves_aVF = np.zeros((((n_divs-1)*(m_divs-1)), tot_iterations-start_recording), dtype=float)
...
for spiral_pos in range(0,((n_divs-1)*(m_divs-1))):
    ...
    Int_ECG_R = np.zeros(tot_iterations-start_recording)
    Int_ECG_L = np.zeros(tot_iterations-start_recording)
    Int_ECG_F = np.zeros(tot_iterations-start_recording)
    Int_ECG_L1 = np.zeros(tot_iterations-start_recording)
    Int_ECG_L2 = np.zeros(tot_iterations-start_recording)
    Int_ECG_L3 = np.zeros(tot_iterations-start_recording)
    Int_ECG_L4 = np.zeros(tot_iterations-start_recording)
    Int_ECG_L5 = np.zeros(tot_iterations-start_recording)
    Int_ECG_L6 = np.zeros(tot_iterations-start_recording)
    ...
    for itern in range(0,tot_iterations):
        ...
        if (itern == 0) | (itern%970 == 0):
            Ugrid2, Ngrid2 = PlaceFib(Ugrid, Ngrid, (Coord_array[spiral_pos][0]-int(Focal_size / 2)), (Coord_array[spiral_pos][0]+int(Focal_size / 2)), (Coord_array[spiral_pos][1]-int(Focal_size / 2)),
                                      (Coord_array[spiral_pos][0]+int(Focal_size / 2)))
...
# PLOTS:
# for plots in range(0,(n_divs-1)*(m_divs-1)):
#     # Plot all ECG traces from all 9 electrodes **ASSUMING ONLY 1 SPIRAL**
#     plt.subplot(1, ((n_divs-1)*(m_divs-1)), plots+1)
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_Lii[plots]), label='Lii')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_Liii[plots]), label='Liii')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_WCT[plots]), label='WCT')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_V1[plots]), label='V1')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_V2[plots]), label='V2')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_V3[plots]), label='V3')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_V4[plots]), label='V4')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_V5[plots]), label='V5')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_V6[plots]), label='V6')
#     plt.plot(np.arange(tot_iterations-start_recording), np.transpose(All_Pwaves_aVF[plots]), label='aVF')
#     plt.legend(loc=0, prop={'size': 6})
#     plt.xlabel('Iteration')
#     plt.ylabel('Voltage / mV')
#     plt.title('Spiral at location: ' + str(Coord_array[plots][0]) + ',' + str(Coord_array[plots][1]))
# plt.show()
...
for i in range(0,(n_divs-1)*(m_divs-1)):
    ...
    filename = 'ECG9trace_FP_' + str(i) + '_iterations_' + str(tot_iterations) + '_position_' + str(Coord_array[i][0]) + 'comma' + str(Coord_array[i][1]) + '_LiiLiiiWTCV1V2V3V4V5V6aVF.txt'
